...
export default function WorldViewport({
  ...
  const shouldRenderStream = streamState?.isLive && streamState?.streamUrl && !streamState.fallbackToImage;
    ...
    >
      ...
      {shouldRenderStream && (
        <div className="absolute inset-0 bg-black flex items-center justify-center">
          <div className="text-center text-white">
            ...
            <p className="text-xs opacity-60 mt-1">Stream ID: {streamState.simulationId}</p>
          </div>
      ...
      {onPointerInteraction && (
        <button
          ...
          className="absolute top-2 right-2 px-2 py-1 rounded-full text-xs backdrop-blur-md transition-all"
          style={{ 
            backgroundColor: isInteractive ? 'rgba(var(--color-primary-rgb), 0.8)' : 'rgba(0, 0, 0, 0.4)',
            color: 'white'
          }}
        >
